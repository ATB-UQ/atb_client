...
def test_list_files(api, client):
    api.get("/molecules/21/files").respond(200, json={"items": [
        {"name": "itp_aa", "media_type": "text/plain", "size": 1200, "cached": True},
        {"name": "pdb_aa_opt", "media_type": "chemical/x-pdb", "size": 800, "cached": False},
    ]})
...
def test_pinned_hash_gone(api, client, tmp_path):
    api.get("/molecules/21/files/itp_aa").respond(410, json={
        "type": "https://atb.uq.edu.au/api/v1/errors/topology-version-gone", "status": 410})
...
def test_bundle_download_extracts(api, client, tmp_path):
    ...
    api.get("/jobs/B1").respond(200, json={"id": "B1", "state": "done",
                                           "result": {"url": "/api/v1/bundles/B1.zip"}})
    api.get("/bundles/B1.zip").respond(200, content=buf.getvalue(),
